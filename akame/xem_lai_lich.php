...
<body>
    ...
    font-size: 32px;
    border-bottom: 50px solid white;">Thông Tin Đã đặt lịch</h1>
	...
	<table>
		...
		<?php
			...
			$sql = "SELECT id, name, phone, gender, stylist, date('y-m-d'), time('h:i:sa') FROM customers";
			$result = $conn->query($sql);
			if ($result->num_rows > 0) {
			
				while($row = $result->fetch_assoc()) {
					echo "<tr><td>" . $row["id"]. "</td><td>" . $row["name"] . "</td><td>". $row["phone"]. "</td><td>". $row["gender"] . "</td><td>". $row["stylist"]. "</td><td>". $row["date('y-m-d')"]. "</td><td>". $row["time('h:i:sa')"]. "</td></tr>";
			}
			echo "</table>";
			} else { 
				echo "0 results"; 
